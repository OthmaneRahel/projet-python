...
def Profil_User(request, user_id):
    ...
    user_ads_list = Annonce.objects.filter(user_id=user_id).order_by('-datePublication')
    favoris = Favoris.objects.filter(user_id=user_id).count()
...
class AuthAdministrateur(View):
    def _get_common_context(self, user):
        ...
        paginator = Paginator(annonces_en_attente, 6)  # 6 annonces par page
