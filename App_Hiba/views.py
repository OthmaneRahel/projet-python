...
from django.views import View
from .models import Administrateur, Favoris, Signal, User, Message,Categorie
    ...
    def post(self, request):
        ...
        if user is not None:
            ...
            if sous_categorie_id:
                ...
                annonces = Annonce.objects.all().order_by('-datePublication')  # 6 dernières annonces par défaut

            annonces_non_signalees = annonces.exclude(id__in=Signal.objects.values('annonce_id'))
            return render(request, 'Utilisateur/homePage.html',{'annonces': annonces_non_signalees,'MEDIA_URL': settings.MEDIA_URL,"categories":sous_categories})  # Redirection après succès
        else:
...
def home(request):
    categories = Categorie.objects.all() # Récupère seulement les catégories parentes
    sous_categorie_id = request.GET.get('sous_categorie')
    
    if sous_categorie_id:
        annonces = Annonce.objects.filter(categorie_id=sous_categorie_id).order_by('-datePublication')
    else:
        annonces = Annonce.objects.all().order_by('-datePublication')[:6]  # 6 dernières annonces par défaut
...
class AuthAdministrateur(View):
    def _get_common_context(self, user):
        ...
        return {
            ...
            'Annonces_en_attentes':Annonce.objects.filter(statut = False),
            'MEDIA_URL': settings.MEDIA_URL,
...
def validation_annonces(request):
    ...
    return render(request, 'Administrateur/Validation_Annonces.html', context)